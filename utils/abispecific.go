...
func CastInputs(funcType reflect.Type, args []string) ([]interface{}, error) {
	...
	for i := 0; i < funcType.NumIn()-2; i++ {
		arg := args[i]
		// string representation of argument to find a match
		switch funcType.In(i + 2).String() {
		case "uint8":
		...
		case "uint256":
			n := new(big.Int)
			n, ok := n.SetString(arg, 10)
			if !ok {
				return nil, fmt.Errorf("can't convert %s to big.Int", arg)
			}
			assertedArgs[i] = n
		case "string":
			assertedArgs[i] = arg
		case "common.Address":
			assertedArgs[i] = common.HexToAddress(arg)
		case "[32]uint8":
			var param [32]uint8
			copy(param[:], arg)
